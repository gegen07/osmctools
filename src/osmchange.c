...
bool hash_getparse(int type, int64_t id, const char *s, const char *fn) {
  ...
  while (*s != 0) {  // still characters to parse
    if (*s == '<') { // a key identifier starts here
      ...
      else if ((l = strzlcmp(s, "<nd ref=\"")) > 0)
        o = 0;
      else if ((l = strzlcmp(s, "<member type=\"node\" ref=\"")) > 0)
        o = 0;
      else if ((l = strzlcmp(s, "<member type=\"way\" ref=\"")) > 0)
        o = 1;
...
static long border__bx1 = 2000000000L, border__by1, border__bx2, border__by2;
...
typedef struct read__struct {
  ...
  bool shortobject; // the last encountered object may be written
                    ...
                    // otherwise it hast to be in the long form: <node> ...
                    // </node>
  ...
  uint64_t typeid; // last encountered object typ and id;
                   // lower bits: id; higher bits: type; mask:
                   // 0x7000000000000000LL; 0x1000000000000000LL: node;
  ...
  for (;;) {                                     // until break;
    if (ri->bufp + read__prefetch >= ri->bufe) { // read buffer is too low
      if (ri->fd >= 0) {                         // still bytes in the file
        ...
        // fprintf(stderr,"read bytes: wanted %u, got
    ...
    if (c == '<') {            // new object
      ...
      if (strzcmp(ri->bufp, "delete>") == 0)
        ...
        ri->section = 3;
      else if (ri->bufp[0] == '/' && (strzcmp(ri->bufp + 1, "delete>") == 0 ||
...
#define read__plausi(t)                                                        \
  if (ri->typeid <= ri->typeidold) {                                           \
    fprintf(stderr, "osmchange: Wrong sequence at " #t " %" PRIi64 " in %s\n", \
            ri->id, ri->fn);                                                   \
  ...
  ri->typeidold = ri->typeid;
        ...
        ret = false;
        break;
      } else if (strzlcmp(ri->bufp, "osm version=\"") && ri->writeflag) {
        char *p;
    ...
    while (--argc > 0) { // for every parameter in command line
      ...
      if (strzcmp(argv[0], "-B=") == 0) {
        ...
        if (!border_file(argv[0] + 3)) {
          fprintf(stderr, "osmchange: No polygon file or too large: %s\n",
                  argv[0]);
          return 4;
