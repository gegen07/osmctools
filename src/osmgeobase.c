...
static void wrs__end() {
  ...
  if (wrs__bufp > wrs__buf) { // at least one character in buffer
    if (write(wrs__fd, wrs__buf, wrs__bufp - wrs__buf) < 0)
      PERRv("flush error at file: %.80s", wrs__name) wrs__bufp =
          wrs__buf; // reset buffer pointer;
...
static bool wrs_open(const char *name) {
  ...
  if (name == NULL) { // standard output
    ...
    if (wrs__fd <= 0) {
      PERRv("could not open output file: %.80s", wrs__name) wrs__fd = -1;
      return false;
    }
  } // not standard output
  return true;
...
static inline void wrs_char(int c) {
  ...
  if (wrs__bufp >= wrs__bufe) { // write buffer is full
    // flush
    if (write(wrs__fd, wrs__buf, wrs__bufp - wrs__buf) < 0)
      PERRv("write error at file: %.80s", wrs__name) wrs__bufp =
          wrs__buf; // reset buffer pointer;
      ...
      if (b < 0x10 || b > 0x12) { // not a regular dataset id
        if (b >= 0xf0) {          // single byte dataset
          if (b == 0xff)          // file start, resp. o5m reset
            ...
            PLOGv(1, "unknown .o5m short dataset id: 0x%02x", b) read_bufp++;
          continue;
        }      // end   single byte dataset
        else { // unknown multibyte dataset
          if (b != 0xe0 && b != 0xdb && b != 0xdc)
            PLOGv(1, "unknown .o5m dataset id: 0x%02x", b) read_bufp++;
    ...
    /* read and process one osm object */ {
      ...
      if (bufp >= bufe)
        ...
        if (otype == 0) { // node
          ...
          const char *region, *subregion, *city, *street, *place, *housenumber,
              *boundary;
          int8_t weight;

          region = subregion = city = street = place = housenumber = boundary =
              NULL;
          weight = 0;
          while (bufp < bufe) {      // each k-v pair
            str_read(&bufp, &k, &v); // read key-val pair
            if (strcmp(k, "region") == 0)
              region = v;
            else if (strcmp(k, "subregion") == 0)
              subregion = v;
            else if (strcmp(k, "addr:city") == 0)
              city = v;
          ...
          if (nodecount == 0) {
            ...
            data_addradd(COORDINATE_NIL, COORDINATE_NIL, NULL, NULL, NULL, NULL,
                         NULL, 0);
            nodecount++;
          }
          ...
          if ((city == NULL && housenumber == NULL) || street != NULL) {
            ...
            if (housenumber == NULL)
              ...
              data_addradd(lon, lat, region, subregion, city, street,
                           housenumber, weight);
              ...
              for (;;) { // for all substrings
                ...
                if (*v0 != 0) {
                  ...
                  if (subsequent)
                    nodecount++;
                  subsequent = true;
                  data_addradd(lon, lat, region, subregion, city, street, v0,
                               weight);
                }
...
struct data__str_struct {
  char string[data__strstringM]; // string (including terminator);
                                 // if maximum string length, the terminator is
                                 // omitted;
  ...
  union {
    ...
    struct {
      uint32_t filler;
      data__strref_t *usedref, *usedlast;
    };
    // if there are more than 'data__strusedM' backreferences,
...
static data__coco_t
    *data__coco; // rounded geocoordinate index;
                 // search criterion is a geocoordinates combination:
                 // ((lat+900000000)&0xffffc000)*2^16 + (lon+1800000000);
                 // 16384 * 100 nanodegrees represents a length of about 182
                 // meters; this is done to accelerate the determination of
                 // neighboring points within a certain geographical distance;
...
static inline data__coco_t *data__coco_find(uint64_t cc) {
  ...
  while (i1 < i2) { // binary search
    i = (i1 + i2) / 2;
    if (cc < data__coco[i].yx)
      i2 = i;
    else
      i1 = i + 1;
  } // binary search
  while (i > 0 && cc == data__coco[i - 1].yx)
    i--;
...
static inline void data__coco_center(int32_t x, int32_t y, int32_t radius) {
  ...
  data__coco_here =
      NULL; // to tell data__coco_next() that the
...
static bool data__addrclean() {
  ...
  while (readp < data__addre) { // for each dataset
    if ((readp->region != readbeforep->region ||
        ...
        (readp->region != 0 || readp->subregion != 0 || readp->city != 0 ||
         readp->street != 0 || readp->housenumber != 0)) {
...
static int T(data__strtypeid, I) =
    0; // data type id;
...
static bool T(data__strclean, I)() {
  ...
  if (ttab == NULL) {
    PERRv("not enough memory for string index translation type-%i.",
          T(data__strtypeid, I)) return false;
  }
...
static bool T(data__write, I)() {
  ...
  int64_t strlenmax; // length of longest string
                     // (without terminating zeros); that's usually 40 because
                     // of limitation in data__str_t;
...
#undef D
  ...
  wrs_int64(strlenmax);                   // length of longest string
                        // (without terminating zeros); that's usually 40
                        // because of limitation in data__str_t;
...
static int64_t data_ini(const char *file) {
  ...
  if (fd < 0) {
    PERRv("could not open input file: %.80s", file) return 0;
  }
  ...
  if (data__nodecount <= 0) {
    PERRv("incorrect node count value in file \"%.80s\": %" PRIiFAST32, file,
          data__nodecount) return 0;
  }
  ...
  data__addr =
      (data__addr_t *)malloc(sizeof(data__addr_t) * data__nodecount + 4);
  if (data__addr == NULL) {
  ...
  /* care about string reference memory area */ {
    ...
    PLOGv(1, "string reference size: %" PRIi64,
          sizeref) if (global_maxstrrefs > 0) // manually-defined absolute size
    ...
    if (sizeref < 128)
      sizeref = 128;
    sizeref -= (sizeref + sizeof(data__strref_t) - 1) % sizeof(data__strref_t);
    ...
    if (data__strref == NULL) {
      PERRv("not enough memory for string references: %" PRIi64 " MiB",
            sizeref / (1024 * 1024))
          PINFO("try to decrease  --max-strrefs=") return 0;
    }
  ...
  data__coco =
      (data__coco_t *)malloc(sizeof(data__coco_t) * data__nodecount + 4);
  if (data__coco == NULL) {
...
static void data_addradd(int32_t x, int32_t y, const char *region,
                         const char *subregion, const char *city,
...
#define D(z)                                                                   \
  si = T(data_index, z)(z);                                                    \
  if (si == 0 && z != NULL)                                                    \
    PERRv("unknown " #z " string: %.80s", z) data__addre->z = si;
  D(region)
  D(subregion)
  D(city)
  D(street)
...
#undef D
  ...
  if (hnr > 65535)
    hnr = 65535; // limit range to fit into 16 bit
...
static bool data_addrcomplete() {
  ...
  /* initialize rounded geocoordinate combination index */ {
    ...
    qsort(data__coco, data__addrn, sizeof(data__coco_t), data__coco_qsort);
  } // initialize rounded geocoordinate combination index
      ...
      if (addrp->city == 0) { // city is unknown
        if (street == 0) {    // street is unknown
          if (addrp->housenumber == 0 && addrp->region != 0) {
            ...
            if (ci == 0)
              PWARNv("missing city string for region %.80s", rs) else {
        ...
        if (addrp->housenumber != 0) {
          ...
          if (ci == 0)
            PWARNv("could not find city string for region %.80s",
                   data_string_region(addrp->region)) else addrp->city = ci;
          continue;
        } // house address, not just a street
          ...
          if (l >= 3 && c != 0 &&
            ...
            if (ci == 0)
              PWARNv("missing city string for region %.80s",
                     data_string_region(addrp->region)) else {
                // take region string for city string;
                addrp->city = ci;
                PLOGv(2, "%.80s -> %.80s\n", cs, rs)
              }
      ...
      if (si != 0) { // there is a subregion
        ...
        if (ci != 0) { // there is a city
          ...
          if (l >= 3) { // contents of subregion string starts
            ...
            if (ci == 0)
              PWARNv("missing city string for subregion %.80s", ss) else {
  ...
  /* order address data by city name, street name, etc. */ {
    qsort(data__addr, data__addrn, sizeof(data__addr[0]), data__addr_qsort);
  }
...
#define data__ini_bufM 2000000 // size of buffer
  ...
  /* count all strings of the file */ {
    ...
    if (buf == NULL) {
      PERRv("not enough memory for analyzing file %.80s", file) return false;
    }
  ...
  fd = open(file, O_RDONLY | O_BINARY);
  if (fd < 0) {
    PERRv("could not reopen input file: %.80s", file) return false;
  }

  // allocate memory for the strings
  T(data__str, I) = (data__str_t *)malloc(sizeof(data__str_t) * strn + 4);
  if (T(data__str, I) == NULL) {
    PERRv("not enough memory for strings of %.80s", file) return false;
  }
  ...
  /* read all strings of the file */ {
    ...
    buf = (char *)malloc(data__ini_bufM + 4);
    if (buf == NULL) {
      PERRv("not enough memory for reading file %.80s", file) return false;
    }
    strread = 0;
    bufp = bufe = buf;
    bufee = buf + data__ini_bufM;
    ...
    for (;;) {                     // for all strings in file
      ...
      if (T(data__stre, I) >= stree) {
        PERRv("string memory overflow: %.80s", file) FREE(buf) close(fd);
        return false;
      }
    ...
    if (strread == 0 && T(data__strtypeid, I) != 2 || 1 + strread != strn) {
      ...
      PWARNv("could not completely read file %.80s", file) PINFOv(
          "read %" PRIuFAST32 " strings of %" PRIuFAST32 ".", strread, strn);
      close(fd);
      return false;
    }
  } // read all strings of the file
